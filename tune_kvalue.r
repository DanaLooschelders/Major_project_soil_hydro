...
for(i in kvalues){
  ...
  swc_temp<-calc_swc_M(P=snowpack$rain, M=snowpack$actual_melt, ET=Hyytiala_all_day$Evapotr,
             max_swc=max_swc_H*1000, k=i, min_swc=min_swc_H*1000)
  swc_temp$sum<-swc_temp$sum/1000
  swc_temp$obs<-Hyytiala_all_day$SWC20 #add observations to sim data
  #calculate RMSE for model
  optimalk_snow$RMSE[optimalk_snow$kvalue==i]<-  sqrt(mean((swc_temp$obs - swc_temp$sum)^2))
  #calculate linear fit
  optimalk_snow$rsquared[optimalk_snow$kvalue==i]<- summary(lm(swc_temp$sum~swc_temp$obs))[["adj.r.squared"]]
}
